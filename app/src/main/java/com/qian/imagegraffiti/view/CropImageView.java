...
import com.qian.imagegraffiti.view.model.HighlightView;
import com.qian.imagegraffiti.view.model.ImageMoveView;
   ...
   // private Bitmap srcBitmap = null;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        case STATE_HIGHLIGHT:
        	...
        	break;
        case STATE_HAND_WRITE:
        	canvas.drawBitmap(HandWriting(mTmpBmp,canvas), 0, 0,null);
        	break;
        }
    }

    public Bitmap HandWriting(Bitmap mTmpBmp, Canvas canvas)
	{
		canvas = new Canvas(mTmpBmp);
		if(isMove){
			canvas.drawLine(startX, startY, clickX, clickY, paint);
		}
		startX = clickX;
		startY = clickY;
		targetBitmap = mTmpBmp;
		return mTmpBmp;
	}
	 ...
	 * 组合手写涂鸦和源图片
	 */
	public Bitmap combinate()
	{
		setState(CropImageView.STATE_NONE);
		return targetBitmap;
	}
	 ...
	 * 取消合并
	 */
	public Bitmap cancelCombinate()
	{
		this.setState(CropImageView.STATE_NONE);
		return mTmpBmp;
	}
    ...
    public void setCurBitmap(Bitmap bitmap){
    	...
    	//srcBitmap = Bitmap.createBitmap(bitmap);
    	mTmpBmp = bitmap.copy(Bitmap.Config.ARGB_8888, true);
    	paint = new Paint();
