...
public class ImageSpecific
{
	...
	public static final int FLAG_ALPHA_CROP = FLAG_OLD_REMEMBER + 9;
...
//			bm = sunshine(bmp);
	...
	private Bitmap overlay(Bitmap bmp)
	{
		...
		Bitmap overlay = BitmapFactory.decodeResource(mContext.getResources(), R.mipmap.rainbow_overlay);
	...
	private Bitmap alphaLayer(Bitmap bmp)
	{
		...
		// 边框图片
		Bitmap overlay = BitmapFactory.decodeResource(mContext.getResources(), R.mipmap.black);
		int w = overlay.getWidth();
		int h = overlay.getHeight();
		float scaleX = width * 1F / w;
		float scaleY = height * 1F / h;
