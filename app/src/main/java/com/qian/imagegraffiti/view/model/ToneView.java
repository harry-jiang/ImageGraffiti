package com.qian.imagegraffiti.view.model;
...
public class ToneView
{
	...
	private void init(Context context)
	{
		...
		mHue.setText(R.string.lightness);//显示字符串为亮度（找了很久找不到色相与亮度相反的bug，只能这样调整了）
		mLum = new TextView(context);
		mLum.setText(R.string.contrast);
		mCon = new TextView(context);
		mCon.setText(R.string.contr);
		...
		mConBar = new SeekBar(context);
		mConBar.setMax(255);
		mConBar.setProgress(127);
		mConBar.setTag(4);
		...
		con.setLayoutParams(new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT));
		
		mCon.setGravity(Gravity.CENTER);
		con.addView(mCon, txtLayoutparams);
		con.addView(mConBar, seekLayoutparams);
	...
	public void setConBarListener(OnSeekBarChangeListener l)
	{
		mConBar.setOnSeekBarChangeListener(l);
	}
	...
	public Bitmap handleImage(Bitmap bm, int flag)
	{
		...
		case 2: // 亮度
			...
			mLightnessMatrix.setRotate(0, mLightnessValue); // 控制让红色区在色轮上旋转hueColor的角度
			mLightnessMatrix.setRotate(1, mLightnessValue); // 控制让绿红色区在色轮上旋转hueColor的角度
			mLightnessMatrix.setRotate(2, mLightnessValue); // 控制让蓝色区在色轮上旋转hueColor的角度
			// 这里相当于改变的是全图的色相
			Log.d("may", "改变亮度");
			break;
		case 3://对比度
